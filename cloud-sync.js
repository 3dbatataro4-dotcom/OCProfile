...
(()=>{
  ...
  function shell(content){const previousPanel=modal.querySelector('.oc-cloud-panel'),scrollTop=previousPanel?.scrollTop||0;modal.innerHTML=`<section class="oc-cloud-panel" role="dialog" aria-modal="true" aria-labelledby="cloud-title"><header><div><small>YOUR PRIVATE ARCHIVE</small><h2 id="cloud-title">雲端存檔</h2></div>${button('close','✕ 關閉')}</header>${content}<p role="status" aria-live="polite"></p></section>`;modal.querySelector('.oc-cloud-panel').scrollTop=scrollTop;}
  function render(){
