...
function loadStateFromLocalStorage() {
  ...
  try { timelines = JSON.parse(localStorage.getItem('oc_timelines') || '[]'); if (!Array.isArray(timelines)) timelines = []; } catch { timelines = []; }
  try { mediaLibrary = JSON.parse(localStorage.getItem('oc_media_library') || '[]'); if (!Array.isArray(mediaLibrary)) mediaLibrary = []; } catch { mediaLibrary = []; }
  ...
  visualNovelTopDown = localStorage.getItem("oc_visual_novel_top_down") === "true";
...
function saveStateToLocalStorage() {
  ...
  put('oc_timelines', timelines);
  put('oc_media_library', mediaLibrary);
  put("oc_collapsed_books", collapsedBooks);
  put("oc_visual_novel_templates", visualNovelTemplates);
  put("oc_visual_novel_top_down", String(visualNovelTopDown));
...
function renderCpModule() {
  ...
  if (!visibleCps.length) {
    grid.innerHTML = `<div class="empty-state"><p>目前沒有符合此篩選條件的關係組合。</p></div>`;
    return;
  }
...
async function saveLocalAvatar(file) {
  if (!/^image\/(png|jpeg|webp|gif|avif)$/i.test(file.type)) throw new Error('請選擇 PNG、JPEG、WebP、GIF 或 AVIF 圖片。');
  if (file.size > 20 * 1024 * 1024) throw new Error('圖片不可超過 20 MB。');
  const id=`avatar_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
  await OCFeatures.storeFile(id,file);
  mediaLibrary.push({id,kind:'avatar',title:file.name,type:file.type,size:file.size,createdAt:Date.now(),localOnly:true});
  saveStateToLocalStorage();
  return `asset:${id}`;
}

async function handleWizAvatarUpload(event) {
  const file = event.target.files[0];
  if (!file) return;
  try{const ref=await saveLocalAvatar(file);document.getElementById("wizAvatarUrl").value=ref;updateWizAvatarPreview(ref);}catch(error){alert(error.message);}
  event.target.value='';
}
...
async function handleCustomAvatarUpload(event) {
  const file = event.target.files[0];
  if (!file) return;
  try{const ref=await saveLocalAvatar(file);document.getElementById("charAvatarUrl").value=ref;updateModalAvatarPreview(ref);}catch(error){alert(error.message);}
  event.target.value='';
}
...
async function addCustomPresetAvatarFile(event){const file=event.target.files?.[0];if(!file)return;try{const url=await saveLocalAvatar(file);customPresetAvatars.push({name:file.name.replace(/\.[^.]+$/,''),url});saveStateToLocalStorage();openAvatarGalleryModal();}catch(error){alert(error.message);}event.target.value='';}
...
function renderFactionList() {
  ...
  container.innerHTML = factions.map(f => {
    const worldColorA=/^#[0-9a-f]{6}$/i.test(f.themeColor?.primary||'')?f.themeColor.primary:'#a56e79';
    const worldColorB=/^#[0-9a-f]{6}$/i.test(f.themeColor?.secondary||'')?f.themeColor.secondary:'#686a9c';
    ...
    return `
      <div class="faction-card" style="--world-a:${worldColorA};--world-b:${worldColorB}">
        <h3>
...
function renderDocumentsModule() {
  ...
  const activeChars = characters.filter(c => !c.isHidden);
  const typedChar=(document.getElementById('docCharSearch')?.value||'').trim().toLocaleLowerCase();
  ...
  if (charFilterSelect) {
    ...
    const exact=activeChars.find(c=>c.name.toLocaleLowerCase()===typedChar);
    const visible=activeChars.filter(c=>!typedChar||c.name.toLocaleLowerCase().includes(typedChar)||c.id===previous);
    charFilterSelect.innerHTML = `<option value="">全部角色</option>` + visible.map(c => `<option value="${escapeHtml(c.id)}">${escapeHtml(c.name)}</option>`).join('');
    charFilterSelect.value = exact?.id || previous;
  }
  ...
  const docSort=document.getElementById('docSort')?.value||'manual';
  const sortByTitle=(a,b)=>docSort==='manual'?0:(docSort==='title' ? 1 : -1)*String(a.title||'').localeCompare(String(b.title||''),'zh-Hant');
  const filteredDocs = documents.filter(filterFn).sort(sortByTitle);
...
function openBookModal(bookId = null) {
  ...
  document.getElementById("bookCharSearch").value = "";
...
function openDocumentModal(docId = null, defaultBookId = "") {
  ...
  document.getElementById("docCharSearchEdit").value = "";
...
function saveVisualNovelScript(preview = false) {
  ...
  doc.visualNovel = { ...(doc.visualNovel || {}), version: 1, eventIndexVersion:2, settings: collectVisualNovelSettings(), scriptText, aiCustomPrompt, updatedAt: new Date().toISOString() };
...
function getVisualNovelScriptLines(scriptText) {
  return formatVisualNovelScriptBlocks(scriptText).split(/\r?\n/).filter(line => line.trim() !== "");
}

function encodeVisualNovelInlineLineBreaks(text) {
  return String(text || "").replace(/\r\n?/g, "\n").replace(/\n/g, "\u2028");
}

function decodeVisualNovelInlineLineBreaks(text) {
  return String(text || "").replace(/\u2028/g, "\n");
}

function migrateVisualNovelEventIndex(doc) {
  if (!doc?.visualNovel || doc.visualNovel.eventIndexVersion === 2) return;
  if (doc.visualNovel.bookmarkIndex != null) doc.visualNovel.bookmarkIndex = Math.floor(Number(doc.visualNovel.bookmarkIndex) / 2);
  doc.visualNovel.eventIndexVersion = 2;
}
...
async function preloadVisualNovelCg(source) {
  let url=String(source||'').trim();try{url=await window.OCVnAssets?.resolve?.(url)||url;}catch{return false;}
...
async function preloadVisualNovelAudio(source) {
  const original = normalizeVisualNovelAudioSource(source);
  let url=original;try{url=await window.OCVnAssets?.resolve?.(original)||original;}catch{return false;}
  if (!url || url.toLowerCase() === "none") return Promise.resolve(false);
...
async function transitionVisualNovelCg(source) {
  ...
  visualNovelPendingCgSource = ref;
  ...
  let url;
  try { url = await window.OCVnAssets?.resolve?.(ref) || ref; } catch(error) { console.warn('CG 素材讀取失敗',error); visualNovelPendingCgSource=''; return; }
  if(requestToken!==visualNovelCgRequestToken)return;
  ...
  if (requestToken !== visualNovelCgRequestToken || visualNovelPendingCgSource !== ref) return;
  void next.offsetWidth;
  layers.forEach((layer, index) => layer.classList.toggle("active", index === nextIndex));
  visualNovelCgLayerIndex = nextIndex;
  visualNovelActiveCgSource = loaded ? ref : "";
  visualNovelPendingCgSource = "";
...
async function transitionVisualNovelBgm(src) {
  ...
  if(source&&source.toLowerCase()!=="none")try{playableSource=await window.OCVnAssets?.resolve?.(source)||source;}catch(error){console.warn('BGM 素材讀取失敗',error);visualNovelPendingBgmSource='';return;}
  ...
  next.pause(); next.src = playableSource; next.dataset.source = source; next.volume = 0; next.muted = current.muted; next.load();
...
async function playVisualNovelAudio(type, src) {
  const source = normalizeVisualNovelAudioSource(src);
  if (type === "bgm") {
...
function visualNovelReadableNameColor(color){
  if(!/^#[0-9a-f]{6}$/i.test(String(color||'')))return '#d7a67d';
  const rgb=[1,3,5].map(index=>parseInt(color.slice(index,index+2),16));
  const luminance=values=>values.map(value=>{const channel=value/255;return channel<=.04045?channel/12.92:((channel+.055)/1.055)**2.4;}).reduce((sum,value,index)=>sum+value*[.2126,.7152,.0722][index],0);
  for(let i=0;i<18&&luminance(rgb)>.45;i++)rgb.forEach((value,index)=>rgb[index]=Math.round(value*.93));
  for(let i=0;i<18&&luminance(rgb)<.19;i++)rgb.forEach((value,index)=>rgb[index]=Math.round(value*.91+255*.09));
...
function executeVisualNovelEvent(event) {
  ...
  if (event.type === "blank") {
    ...
    insertVisualNovelFeedNode(document.getElementById("vnStoryFeed"), spacer);
    return false;
  }
  ...
  const speakerTextColor=visualNovelReadableNameColor(currentVisualNovelSettings.useCharacterColors!==false&&character?.themeColor?.primary?character.themeColor.primary:currentVisualNovelSettings.primaryColor||'#d97706');
  ...
  if (narrator) {
    ...
    card.style.setProperty("--speaker-text-color",speakerTextColor);
  ...
  insertVisualNovelFeedNode(feed, row);
  focusVisualNovelEntryTop(feed, row);
...
function focusVisualNovelEntryTop(feed, row) {
  if (!feed || !row || visualNovelSuppressEntryScroll) return;
  const token = ++visualNovelEntryScrollToken;
...
function insertVisualNovelFeedNode(feed, node) {
  if (!feed || !node) return;
  if (visualNovelTopDown) {
    const heading = feed.querySelector(":scope > .vn-feed-chapter");
    feed.insertBefore(node, heading?.nextSibling || feed.firstChild);
...
function rebuildVisualNovelPlaybackAfterScriptEdit(readingIndex=currentVisualNovelIndex){
  ...
  finishVisualNovelTyping(true);currentVisualNovelSettings.typewriterEnabled=typewriter;currentVisualNovelIndex=target;feed.scrollTop=visualNovelTopDown?0:feed.scrollHeight;updateVisualNovelBookmarkBtnUI();
}
...
function openVnSpeakerEditor(eventIndex,row){
  ...
  select.value=[...select.options].some(option=>option.value===event.speaker)?event.speaker:'__custom__';document.getElementById('vnSpeakerCustomInput').value=event.speaker||'';document.getElementById('vnSpeakerTextInput').value=event.text||'';const modal=document.getElementById('vnSpeakerEditorModal');modal.style.setProperty('--vn-primary',currentVisualNovelSettings.primaryColor||'#d97706');modal.style.setProperty('--vn-secondary',currentVisualNovelSettings.secondaryColor||currentVisualNovelSettings.primaryColor||'#7c3aed');modal.classList.add('active');
}
...
function saveVnSpeakerCorrection(){
  ...
  currentVisualNovelEvents=buildVerifiedVisualNovelEvents(doc.visualNovel.scriptText);
  replaceRenderedVisualNovelDialogue(eventIndex);
  closeVnSpeakerEditor();showVnFloatingToast('說話人與對話內文已更正並保存');
}

function replaceRenderedVisualNovelDialogue(eventIndex) {
  const feed=document.getElementById('vnStoryFeed'),oldRow=feed?.querySelector(`.vn-feed-entry[data-event-index="${eventIndex}"]`),updated=currentVisualNovelEvents[eventIndex];
  if(!feed||!oldRow||updated?.type!=='dialogue')return;
  const anchor=oldRow.nextSibling,savedIndex=currentVisualNovelIndex,typewriter=currentVisualNovelSettings.typewriterEnabled,oldTop=feed.scrollTop;
  oldRow.remove();currentVisualNovelIndex=eventIndex;currentVisualNovelSettings.typewriterEnabled=false;visualNovelSuppressEntryScroll=true;executeVisualNovelEvent(updated);visualNovelSuppressEntryScroll=false;
  const newRow=feed.querySelector(`.vn-feed-entry[data-event-index="${eventIndex}"]`);
  if(newRow&&anchor?.parentNode===feed)feed.insertBefore(newRow,anchor);
  currentVisualNovelSettings.typewriterEnabled=typewriter;currentVisualNovelIndex=savedIndex;
  const key=`${currentVisualNovelDocId}:${eventIndex}`,history=visualNovelHistory.find(item=>item.key===key);if(history){history.speaker=updated.speaker;history.text=updated.text;}
...
function deleteVnCurrentSentence(){
  ...
  const feed=document.getElementById('vnStoryFeed');feed?.querySelector(`.vn-feed-entry[data-event-index="${eventIndex}"]`)?.remove();
  currentVisualNovelEvents=buildVerifiedVisualNovelEvents(documents.find(item=>item.id===currentVisualNovelDocId)?.visualNovel?.scriptText||'');
  feed?.querySelectorAll('.vn-feed-entry[data-event-index]').forEach(row=>{const index=Number(row.dataset.eventIndex);if(index>eventIndex){row.dataset.eventIndex=index-1;row.dataset.sourceLineIndex=Math.max(0,Number(row.dataset.sourceLineIndex||index)-1);}});
  visualNovelHistory=visualNovelHistory.filter(item=>item.key!==`${currentVisualNovelDocId}:${eventIndex}`).map(item=>{const [docId,indexText]=item.key.split(':');const index=Number(indexText);if(docId===currentVisualNovelDocId&&index>eventIndex)item.key=`${docId}:${index-1}`;return item;});
  if(currentVisualNovelIndex>=eventIndex)currentVisualNovelIndex=Math.max(-1,currentVisualNovelIndex-1);ensureVisualNovelCgMatchesScript(currentVisualNovelIndex);saveStateToLocalStorage();closeVnSpeakerEditor();showVnFloatingToast('這一句已刪除');
}

function handleVisualNovelStageClick(event){if(visualNovelScriptEditMode)return;advanceVisualNovel(event);}
function toggleVisualNovelScriptEditMode(event){event?.stopPropagation?.();visualNovelScriptEditMode=!visualNovelScriptEditMode;visualNovelAutoPlay=false;clearTimeout(visualNovelAutoTimer);document.getElementById('vnAutoPlayBtn')?.classList.remove('active');document.getElementById('vnPlayer').classList.toggle('vn-script-editing',visualNovelScriptEditMode);const button=document.getElementById('vnScriptEditModeBtn'),full=document.getElementById('vnOpenFullScriptEditorBtn'),repair=document.getElementById('vnScriptRepairTools');button.innerHTML=visualNovelScriptEditMode?'<i class="fa-solid fa-check"></i> 結束編輯劇本模式':'<i class="fa-solid fa-pen-to-square"></i> 開啟編輯劇本模式';full.style.display=visualNovelScriptEditMode?'flex':'none';if(repair)repair.hidden=!visualNovelScriptEditMode;showVnFloatingToast(visualNovelScriptEditMode?'編輯劇本模式：雙擊／雙點頭像或對話框即可校對':'已結束編輯劇本模式');}
...
function updateVisualNovelSettingsUI() {
  ...
  const topDownCheck = document.getElementById("vnTopDownCheck");
  if (topDownCheck) topDownCheck.checked = visualNovelTopDown;
}

function toggleVisualNovelTopDown(event) {
  event?.stopPropagation?.();
  visualNovelTopDown = !!document.getElementById("vnTopDownCheck")?.checked;
  saveStateToLocalStorage();
  rebuildVisualNovelPlaybackAfterScriptEdit(currentVisualNovelIndex);
  showVnFloatingToast(visualNovelTopDown ? "已改為從上往下閱讀" : "已改為從下往上閱讀");
}
...
function renderExportCharList() {
  ...
  if (container) {
    ...
    `).join('');
    window.OCFeatures?.filterChoices?.('exportCharSearch','exportCharList');
  }
...
async function generateExportText() {
  ...
  let text = "";
  if (mode === 'timelines_only') {
    document.getElementById('exportPreviewArea').value = window.OCFeatures?.timelineText?.() || '';
    return;
  }
  ...
  if (mode === 'full' && document.getElementById('expIncTimelines')?.checked) text += '\n' + (window.OCFeatures?.timelineText?.() || '');
...
function applyCloudSyncString() {
  ...
  try {
    ...
    if (Array.isArray(data.timelines)) timelines = data.timelines;
    if (Array.isArray(data.mediaLibrary)) mediaLibrary = data.mediaLibrary;
...
function applyWorkshopBackupData(data) {
  ...
  normalizeVisualNovelDocuments();
  if (data.visualNovelTemplates) visualNovelTemplates = data.visualNovelTemplates;
  if (typeof data.visualNovelTopDown === 'boolean') visualNovelTopDown = data.visualNovelTopDown;
  if (Array.isArray(data.customPresetAvatars)) customPresetAvatars = data.customPresetAvatars;
  if (data.collapsedBooks) collapsedBooks = data.collapsedBooks;
...
function openApiKeyModal() {
  ...
  window.OCFeatures?.populateApiSettings?.();
}
...
function saveApiKeySettings() {
  ...
  if (window.OCFeatures?.saveApiSettings && !window.OCFeatures.saveApiSettings()) { alert("請輸入有效的 HTTPS Worker 網址。"); return; }
