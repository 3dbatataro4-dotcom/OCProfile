...
保留已有用戶的性格與記憶，依互動對象的帳號 ID 和官方／同人身分作出反應。記憶與關係更新只記錄本次實際互動，勿杜撰不存在的歷史。不要替人類控制的 owned 帳號發言。只有dynamicName=true的用戶（約10%）可提供displaySuffix，依本次內容與喜歡的角色換一句動態應援梗，例如今天也在為XX打摳、我要當XXX的狗，不含豎線與固定名稱。其他用戶displaySuffix留空，保持一般暱稱。這是同一用戶換展示句，不能建立新ID或冒充其他人。`;
  ...
  async function action(full) {
    ...
    if(a==='save-post-edit'){
      ...
      Object.assign(p,{title,content,note:val('ff-edit-post-note').trim(),boardId:val('ff-edit-post-board'),type:p.kind==='book'?'書籍':val('ff-edit-post-type'),tags:C.tags(val('ff-edit-post-tags')),charIds:picks('ff-edit-post-chars'),fan:checked('ff-edit-post-fan'),updatedAt});
